/*
Create on Mon Aug 08 06:22:32 EDT 2022
*Copyright (C) 122.
@author Alejandro subero
@author JSH
@author  
@since 11.0
@version1.0.0.0
@version  %I%, %G%
*<p>Description: Employee’s hamblet </p>
*/


package com.js.employee.validation;

import java.util.Optional;
import java.util.ArrayList;
import java.util.List;
import java.util.Date;

import com.js.employee.entitys.Empleado;

import java.util.regex.Pattern;

import org.springframework.stereotype.Service;
import com.js.employee.pojo.EmpleadoPojo;
import com.js.employee.pojo.EmpleadoPojo;
import com.js.employee.entitys.FichaDesempeno;
import com.js.employee.pojo.FichaDesempenoPojo;
import com.js.employee.entitys.FichaPermisos;
import com.js.employee.pojo.FichaPermisosPojo;
import com.js.employee.entitys.FichaSueldo;
import com.js.employee.pojo.FichaSueldoPojo;
import com.js.employee.entitys.FichaPagos;
import com.js.employee.pojo.FichaPagosPojo;
import com.js.employee.entitys.FichaFaltas;
import com.js.employee.pojo.FichaFaltasPojo;
import com.js.employee.entitys.FichaMedica;
import com.js.employee.pojo.FichaMedicaPojo;


@Service
public class EmpleadoValidation {

    public EmpleadoPojo valida(EmpleadoPojo empleado) {
        EmpleadoPojo pojo = null;
        try {
            if (empleado != null) {
                if (empleado.getSocialSecurityNumber() != null &&
                                empleado.getName() != null &&
                                empleado.getLastName() != null &&
                                empleado.getAddress() != null &&
                                empleado.getPhoneNumber() != null &&
                                empleado.getPosition() != null &&
                                empleado.getFechaIngreso() != null &&
                                empleado.getComputerSheet() != null &&
                                empleado.getinformacionMedica() != null &&
                                empleado.getsueldo() != null &&
                                empleado.getSocialSecurityNumber() != null
                ) {
                    pojo = empleado;
                }
            }
            return pojo;
        } catch (Exception e) {
            e.printStackTrace();
            return pojo;
        }
    }

    public List<EmpleadoPojo> validaListEmpleados(List<EmpleadoPojo> empleados){
        List<EmpleadoPojo> listEmpleados = new ArrayList<>();

        if(empleados != null && empleados.size() > 0){
            empleados.forEach(empleadoPojo -> listEmpleados.add(this.valida(empleadoPojo)));
        }
        return listEmpleados;
    }

    public EmpleadoPojo validaII(EmpleadoPojo empleado) {
        EmpleadoPojo pojo = null;
        try {
            if (empleado != null) {
                if (
                        empleado.getSocialSecurityNumber() != null &&
                                empleado.getName() != null &&
                                empleado.getLastName() != null &&
                                empleado.getAddress() != null &&
                                empleado.getPhoneNumber() != null &&
                                empleado.getPosition() != null &&
                                empleado.getFechaIngreso() != null &&
                                empleado.getFechaEgreso() != null &&
                                empleado.getCausaEgreso() != null &&
                                empleado.getTotalDiasTrabajados() != null &&
                                empleado.getTotalHorasMes() != null &&
                                empleado.getTotalHorasYear() != null &&
                                empleado.getTotalHorasExtrasYear() != null &&
                                empleado.getDateLastPay() != null &&
                                empleado.getTotalHorasFeriadoYear() != null
                ) {
                    pojo = empleado;
                }
            }
            return pojo;
        } catch (Exception e) {
            e.printStackTrace();
            return pojo;
        }
    }

    // remplace de name of variable for you proyecte
    public Long valida_id(String poder) {
        Long id_Delete = 0l;
        try {
            if (poder != null) {
                if (poder.length() > 0 && !Pattern.matches("[a-zA-Z]+", poder)) {
                    id_Delete = Long.parseLong(poder);
                }
            }
            return id_Delete;
        } catch (Exception e) {
            e.printStackTrace();
            return id_Delete;
        }
    }

    public <T> Object validation(T t) {
        T elemento = null;
        try {
            if (t != null) {
                elemento = t;
            }
            return elemento;
        } catch (Exception e) {
            e.printStackTrace();
            return elemento;
        }
    }
}
 /*
 Copyright (C) 2008 Google Inc.
* Licensed to the Apache Software Foundation (ASF) under one or more
* contributor license agreements.  See the NOTICE file distributed with
* this work for additional information regarding copyright ownership.
* The ASF licenses this file to You under the Apache License, Version 2.0
* (the "License"); you may not use this file except in compliance with
* the License.  You may obtain a copy of the License at
*
*      http://www.apache.org/licenses/LICENSE-2.0
*
* Unless required by applicable law or agreed to in writing, software
* distributed under the License is distributed on an "AS IS" BASIS,
* WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
* See the License for the specific language governing permissions and
* limitations under the License.
*/


